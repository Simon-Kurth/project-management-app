...
const HEALTH_COLORS = ["#10b981", "#f59e0b", "#f43f5e", "#6366f1"];
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">Account health, ticket volume, and renewal pipeline</p>
      </div>
        ...
        <div className="bg-card border border-border rounded-xl p-5">
          ...
          <ResponsiveContainer width="100%" height={220}>
            <BarChart data={data.ticketVolume}>
              ...
              <Bar dataKey="escalated" fill="#f43f5e" name="Escalated" radius={[3, 3, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </div>
              ...
              {data.accountHealth.map((seg, i) => (
                <div key={seg.segment} className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm text-foreground">{seg.segment}</span>
                  </div>
                  <div className="text-right">
                    <span className="text-sm font-semibold text-foreground">{seg.count.toLocaleString()}</span>
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {data.accounts.map((acc) => (
                <tr key={acc.name} className="border-b border-border/50 hover:bg-muted/20 transition-colors">
                  <td className="py-3 px-3 font-medium text-foreground">{acc.name}</td>
                  <td className="py-3 px-3 text-foreground">${(acc.arr / 1000).toFixed(0)}K</td>
                  ...
                  <td className="py-3 px-3 text-foreground text-xs">{acc.csm}</td>
                  <td className="py-3 px-3 text-xs text-muted-foreground">{acc.renewalDate}</td>

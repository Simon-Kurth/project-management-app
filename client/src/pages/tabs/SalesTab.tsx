...
const STAGE_COLORS = ["#6366f1", "#22d3ee", "#f59e0b", "#10b981", "#f43f5e"];
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">Pipeline, rep performance, and revenue forecast</p>
      </div>
        ...
        <div className="bg-card border border-border rounded-xl p-5">
          ...
          <ResponsiveContainer width="100%" height={220}>
            <LineChart data={data.forecastVsActual}>
              ...
              <Tooltip formatter={(v: number) => [`$${(v / 1000).toFixed(0)}K`]} />
              <Legend />
              <Line type="monotone" dataKey="forecast" stroke="#6366f1" strokeWidth={2} strokeDasharray="4 4" name="Forecast" dot={false} />
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {data.repPerformance.map((rep) => (
                <tr key={rep.name} className="border-b border-border/50 hover:bg-muted/20 transition-colors">
                  <td className="py-3 px-3 font-medium text-foreground">{rep.name}</td>
                  <td className="py-3 px-3 text-muted-foreground">${(rep.quota / 1000).toFixed(0)}K</td>
                  <td className="py-3 px-3 text-foreground">${(rep.achieved / 1000).toFixed(0)}K</td>
                  ...
                  <td className="py-3 px-3 text-foreground">{rep.deals}</td>
                  <td className="py-3 px-3">

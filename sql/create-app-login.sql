...
--   the Project Management App Node.js application.  The login is granted
...
--
--   vault kv put secret/pm/db \
--     app_password="$(openssl rand -base64 32)"
--
-- Retrieve at execution time:
--
--   DB_PASS=$(vault kv get -field=app_password secret/pm/db)
--
...
DECLARE @loginName  NVARCHAR(128) = N'pm_app';
...
BEGIN
  DECLARE @sql NVARCHAR(MAX) =
    ...
    N', DEFAULT_DATABASE = ProjectManagement' +
    N', CHECK_EXPIRATION = OFF' +   -- disable expiry for service accounts
    N', CHECK_POLICY = ON;';        -- enforce OS password complexity policy
  EXEC sp_executesql @sql;
  PRINT 'Login pm_app created.';
END
ELSE
BEGIN
  PRINT 'Login pm_app already exists — skipping creation.';
END
GO

-- ============================================================
-- SECTION 2 — DATABASE USER & PERMISSIONS  (run connected to ProjectManagement)
-- ============================================================
...
IF NOT EXISTS (SELECT 1 FROM sys.database_principals WHERE name = N'pm_app')
BEGIN
  CREATE USER pm_app FOR LOGIN pm_app
    WITH DEFAULT_SCHEMA = dbo;
  PRINT 'Database user pm_app created.';
END
ELSE
BEGIN
  PRINT 'Database user pm_app already exists — skipping creation.';
END
GO
...
GRANT SELECT, INSERT, UPDATE, DELETE ON dbo.users               TO pm_app;
GRANT SELECT, INSERT, UPDATE, DELETE ON dbo.refresh_tokens      TO pm_app;
GRANT SELECT, INSERT, UPDATE, DELETE ON dbo.audit_logs          TO pm_app;
GRANT SELECT, INSERT, UPDATE, DELETE ON dbo.dashboard_snapshots TO pm_app;
GRANT SELECT, INSERT, UPDATE, DELETE ON dbo.connector_configs   TO pm_app;
GRANT SELECT, INSERT, UPDATE, DELETE ON dbo.computed_kpis       TO pm_app;
GRANT SELECT, INSERT, UPDATE, DELETE ON dbo.duo_state_store     TO pm_app;
GRANT SELECT, INSERT, UPDATE, DELETE ON dbo.pending_auth_store  TO pm_app;
GO

-- Grant EXECUTE on maintenance stored procedures
GRANT EXECUTE ON dbo.usp_PurgeExpiredTokens    TO pm_app;
GRANT EXECUTE ON dbo.usp_PurgeOldAuditLogs     TO pm_app;
GRANT EXECUTE ON dbo.usp_PurgeExpiredSnapshots TO pm_app;
GO
...
--   2. Connect to the Project Management App database as an Entra admin.

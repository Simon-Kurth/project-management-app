...
This folder contains all hand-authored SQL scripts for the Project Management App database. Scripts are written for **Microsoft SQL Server 2019+** and **Azure SQL Database / Managed Instance**.
...
|---|---|---|---|
| `create-database.sql` | Creates the Project Management App database, all 8 tables, 11 indexes, constraints, 3 triggers, 3 stored procedures, and connector seed data | Once, on a fresh SQL Server instance before first application boot | `sa` or `dbcreator` role |
| `create-app-login.sql` | Creates the `pm_app` least-privilege SQL login and grants only `SELECT/INSERT/UPDATE/DELETE` on application tables plus `EXECUTE` on maintenance procedures. Includes Azure Key Vault, HashiCorp Vault, and CI/CD pipeline retrieval patterns for the password. | Once, after `create-database.sql` | `sa` or `securityadmin` role |
| `maint-purge-agent-job.sql` | Configures Database Mail (SMTP), creates the `ProjectManagement DBA` email operator, and creates 3 SQL Server Agent jobs with failure email alerts: token purge (every 15 min), snapshot purge (hourly), audit log purge (monthly) | Once, after `create-database.sql` | `sa` or `SQLAgentOperatorRole` in msdb |
| `migrate-template.sql` | Reusable template for future schema migrations — includes pre-flight checks, forward migration, rollback, and verification sections | Copy and rename for each new migration | `pm_app` or `db_ddladmin` |
| `migrate-20260401-add-notifications-table.sql` | **First real migration.** Adds `dbo.notifications` table (9 columns, 3 indexes, FK to users), grants permissions to `pm_app`, creates `usp_PurgeOldNotifications` stored procedure, and adds a weekly Agent job for 90-day retention | When deploying the in-app notification centre feature (WH-142) | `pm_app` or `db_ddladmin` |
...
4. Set `DATABASE_URL` in your environment to use the `pm_app` credentials
...
DB_PASS=$(az keyvault secret show --vault-name "pm-kv" \
          --name "pm-app-db-password" --query "value" -o tsv)
...
# Step 4 — Run migrations (connect to ProjectManagement)
sqlcmd -S your-server\INSTANCE -U pm_app -P "$DB_PASS" -d ProjectManagement \
...
2. Connect directly to the `ProjectManagement` database and run `create-database.sql`.
3. Run `create-app-login.sql` Section 1 from the `master` connection, then Section 2 from the `ProjectManagement` connection.
...
*Last updated: April 2026 · Project Management App*
